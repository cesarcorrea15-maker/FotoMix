  ...
  function initializeControls() {
    ...
    // Canvas toolbar: Negrita -> alterna negrita en el título de la vista previa
      ...
      if (DOM.canvasTextBold) {
        DOM.canvasTextBold.addEventListener('click', () => {
          const isActive = false; // state.text.bold; // toggle handled in render loop for accurate preview state
          ...
          try {
              ...
              if(seleccionComponente == 1)//Titulo
                ...
                {
                  state.subtitle.bold = !state.subtitle.bold;
                  isActive = state.subtitle.bold;
                  const previewSelectors = ['.preview-panel>.section-title', '.preview-panel>.section-label'];
                  document.querySelectorAll(previewSelectors.join(',')).forEach(el => {
                    el.classList.toggle('text-bold-active', isActive);
                    el.style.fontWeight = isActive ? '800' : '';
                ...
                {
                    state.description.bold = !state.description.bold;
                    isActive = state.description.bold;
                    const previewSelectors = ['.preview-panel>.section-title', '.preview-panel>.section-label'];
                    document.querySelectorAll(previewSelectors.join(',')).forEach(el => {
                      el.classList.toggle('text-bold-active', isActive);
                      el.style.fontWeight = isActive ? '800' : '';
    ...
    if (DOM.canvasTextSizeInc) {
      DOM.canvasTextSizeInc.addEventListener('click', () => {
                ...
                {
                  state.subtitle.size = clamp(state.subtitle.size + 2, 16, 96);
                  state.overlayPinned = true;
                  if (DOM.textSizeSlider) DOM.textSizeSlider.value = String(state.subtitle.size);
                  if (DOM.previewTextSize) DOM.previewTextSize.value = String(state.subtitle.size);
                  updateText(DOM.textSizeValue, `${state.subtitle.size}px`);
                }
                else if(seleccionComponente == 2)//Descripción
                {
                  state.description.size = clamp(state.description.size + 2, 16, 96);
                  state.overlayPinned = true;
                  if (DOM.textSizeSlider) DOM.textSizeSlider.value = String(state.description.size);
                  if (DOM.previewTextSize) DOM.previewTextSize.value = String(state.description.size);
                  updateText(DOM.textSizeValue, `${state.description.size}px`);
                }
    ...
    if (DOM.canvasTextColorInput) {
        DOM.canvasTextColorInput.addEventListener('change', event => {
          ...
          const isActive = false; // state.text.color && state.text.color !== '#ffffff';  
          ...
          try {
                ...
                {
                  state.subtitle.color = event.target.value;
                  const isActive = state.subtitle.color && state.subtitle.color !== '#ffffff';
                  const previewSelectors = ['.preview-panel>.section-title'];
                  document.querySelectorAll(previewSelectors.join(',')).forEach(el => {
                    el.classList.toggle('text-color-active', isActive);
                    el.style.color = isActive ? state.subtitle.color : '#000000';
                  });     
                }
                else if(seleccionComponente == 2)//Descripción
                {
                  state.description.color = event.target.value;
                  const isActive = state.description.color && state.description.color !== '#ffffff';
                  const previewSelectors = ['.preview-panel>.section-title'];
                  document.querySelectorAll(previewSelectors.join(',')).forEach(el => {
                    el.classList.toggle('text-color-active', isActive);
                    el.style.color = isActive ? state.description.color : '#000000';
                  });
                }
